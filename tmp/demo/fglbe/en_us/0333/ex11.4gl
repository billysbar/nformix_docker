...
#######################################################################
# APPLICATION: Example 11 - 4GL Examples Manual                       # 
#                                                                     # 
# FILE: ex11.4gl                            FORM: f_orders.per,       # 
#                                                 f_custsel.per,      # 
#                                                 f_stocksel.per,     # 
#                                                 f_ship.per          # 
#                                                                     # 
...
#                                                                     # 
# PURPOSE: This program illustrates how to program a single-row       #
#          master form with a multi-row detail form.                  #
#                                                                     # 
# STATEMENTS:                                                         # 
#          BEGIN WORK		  COMMIT WORK                         # 
#          ROLLBACK WORK          EXIT INPUT                          #
#                                                                     # 
# FUNCTIONS:                                                          #
#   add_order() - calls routines needed to add an order.              #
#   input_cust() - accepts user input for customer info.              #
#   cust_popup() - displays a popup window for customers.             #
#   input_order() - accepts user input for order info.                #
#   input_items() - accepts user input for items ordered.             #
#   renum_items() - renumbers the item numbers when an item is added  #
#      or deleted in the items array.                                 #
#   stock_popup(stock_item) - displays a popup window for stock nums  #
...
#   order_amount() - calculates the total cost of the order.          #
#   tax_rates(state_code) - see description in ex4.4gl file.          #
#   ship_order() - opens shipping screen for ship info.               #
#   input_ship() - accepts user input for shipping info.              #
#   order_tx() - performs database operations to add the order in a   #
#     single transaction.                                             #
#   insert_order() - adds "order" row to database.                    #
#   insert_items() - adds associated "items" row(s) to the database.  #
...
#  --------    ----------      -------------------------------------- #
#  02/14/91    dam             Updated the header                     #
...
--* Define a help message file for use with forms in program.
--*  Set forms to start on 2nd line of window. Set Comment and Message
--*  lines to work with f_orders form.
------------------------
  OPTIONS
    HELP FILE "hlpmsgs",
    FORM LINE 2,
...
--* Allow user to add a new order on f_orders
  CALL add_order()
...
--* Order number is not assigned until orders row is saved (order_num
--*   is a SERIAL column). Show user the new order number.
------------------------
